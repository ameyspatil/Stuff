...
class RegressionTesting < Test::Unit::TestCase
    ...
    def setup
        ...
        if $args[2] == "iphone"; @driver.manage.window.resize_to(480, 725) end
    ...
    def test_jobapplications
     ...
     def test_linksnotpresent
        @driver.manage.timeouts.implicit_wait = 1
        begin
                assert @driver.find_element(:link, "About Us"),"The About US link is displayed"
                verify {assert_match true, false , "About us Link is displayed"}
        rescue
                verify { assert true,"The About Us link is not displayed"}
        end
        begin
                assert @driver.find_element(:link, "FAQ"),"The Jibe FAQ Page loads"
                verify {assert_match true, false , "Jibe FAQ link is displayed"}
        rescue
                verify { assert true,"The FAQ link is not displayed"}
        end
        begin
                assert @driver.find_element(:link, "Contact Us"),"The Jibe Commitment Page loads"
                verify {assert_match true, false , "Contact Us link is displayed"}
        rescue
                verify { assert true,"The Contact Us link is not displayed"}
        end
        begin
                assert @driver.find_element(:link, "Privacy Policy"),"The Jibe Commitment Page loads"
                verify {assert_match true, false , "Privacy Policy link is displayed"}
        rescue
                verify { assert true,"The Privacy Policy link is not displayed"}
        end
        begin
                assert @driver.find_element(:link, "Terms of Use"),"The Jibe Commitment Page loads"
                verify {assert_match true, false , "Terms of Use link is displayed"}
        rescue
                verify { assert true,"The Terms of Use link is not displayed exist"}
        end
        begin
                assert @driver.find_element(:name, "location"),"The location text field search is present"
                verify {assert_match true, false , "The location text field search is present on Iphone page width"}
        rescue
                verify { assert true,"The location text field search is present"}
        end
        @driver.find_element(:name,"q").send_keys("Amazon")
        @driver.find_element(:name, "q").send_keys [:return]
        begin
                assert @driver.find_element(:link, "Ask JIBE").displayed?
                verify {assert_match true, false , "Ask JIbe Link is present"}
        rescue
                verify { assert true,"The Ask Jibe twitter link is visible"}
        end
    end
...
test_linksnotpresent = Test::Unit::TestSuite.new("Test Iphone dimensions") << RegressionTesting.new('test_linksnotpresent')
...
t1 = Thread.new { Test::Unit::UI::Console::TestRunner.run(test_jobapplications) }
...
Thread.new { Test::Unit::UI::Console::TestRunner.run(test_facebooklogin) }

t1.join
if $args[2] == "iphone" 
    t2= Thread.new { Test::Unit::UI::Console::TestRunner.run(test_linksnotpresent) }
    t2.join
end
